    ...
    decode_instruction(memwb_reg.instr); // Always decode, even for NOPs
#endif

if (!memwb_reg.valid || memwb_reg.instr == 0x00000013) {
    return;
}
        ...
        **/
        void cycle_pipeline(regfile_t *regfile_p, Byte *memory_p, Cache *cache_p,
            ...
            pregs_p->idex_preg.inp  = stage_decode(pregs_p->ifid_preg.out, pwires_p, regfile_p);
                // --- Forwarding Logic ---
                // Manually update pwires->memwb_wdata from memwb_reg.out
        if (pregs_p->memwb_preg.out.reg_write && pregs_p->memwb_preg.out.rd != 0) {
            pwires_p->memwb_wdata = pregs_p->memwb_preg.out.mem_to_reg ?
                                    pregs_p->memwb_preg.out.mem_data :
                                    pregs_p->memwb_preg.out.alu_result;
        }

        // Now generate forwarding control signals
            ...
            pregs_p->ifid_preg.inp.instr  = 0x00000013; // NOP
            pregs_p->ifid_preg.out.valid  = true;
            pregs_p->ifid_preg.out.instr  = 0x00000013; // NOP
            // KEEP addr
            // pregs_p->ifid_preg.inp.instr_addr = pregs_p->ifid_preg.inp.instr_addr;
            // pregs_p->ifid_preg.out.instr_addr = pregs_p->ifid_preg.out.instr_addr;
